...
(function (undefined) {
	...
	class VisGadget {
		constructor(opt) {
			...
			this.options = VisGadget.defaults;

			util.extent(this.options,opt);
			...
			document.body.setAttribute('visgadget-theme', 'default');
		}
		...
		createView() {
			...
			if (!viewOption.width) {
				viewOption.width = viewOption.height * (2.5 / 7);
			}
			...
			body.style.top = targetRect.top + util.GetScrollTop() + 'px';
			body.style.left = targetRect.left + targetRect.width /*- viewOption.width*/ + 'px';
		...
		createDefaultButton() {
			...
			let selectMode = ['Pointer','Rect','Ellipse','Lasso','Straight','Polyline','FreeSelect'];

			for(let name in this.options.view.defButton){
				let d = this.options.view.defButton[name];
				let i = Object.keys(this.options.view.defButton).indexOf(name);
				...
				//添加SVG元素
				let svgDom = util.ParseSVG(d.svgStr, 'white');
				...
				for(let monitor of this.monitors){
					if(selectMode.indexOf(name)>=0){
						button.onclick = ()=>{eval('monitor.' + name + 'Mode();')};
					}else{
						button.onclick = ()=>{eval('monitor.' + name + '();')};
					}
				}
		...
		filter(opt){
			if(opt.monitorId == null){
				this.monitors.forEach((monitor,i) =>{
					monitor.setFilter(opt.typename,opt.func);
				})
			}else{
				if(this.monitors.length - 1>opt.monitorId){
					this.monitors[opt.monitorId].setFilter(opt.typename,opt.func);
				}
			}
	...
	VisGadget.defaults = {
		...
		view: {
			...
			columns: 3, //按钮列
			button: [], //按钮列表
			defaultButtonColor: 'black', //默认按钮颜色
			defButton:{
				Pointer:{
