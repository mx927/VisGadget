...
(function (undefined) {
	...
	VisGadget.prototype = {
		...
		_initial: function (opt) {
			let def = JSON.parse(JSON.stringify(this._defaultOption));

			// 合并参数
			if (!!opt.view) {
				opt.view = extend(opt.view, def.view, false);
			}

			this.options = extend(def, opt, true);
		...
		createView: function () {
			...
			topBar.ondrag = function (ev) {
				...
				if (tx < 0 || ty < 0) return;
				let pos = viewDock(tx, ty, body.getBoundingClientRect(), document.body.getBoundingClientRect());
				body.style.left = pos[0] + 'px';
				body.style.top = pos[1] + 'px';
			}
			topBar.ondragend = function (ev) {
				let pos = viewDock(ev.clientX - body.dx, ev.clientY - body.dy, body.getBoundingClientRect(), document.body.getBoundingClientRect());
				body.style.left = pos[0] + 'px';
				body.style.top = pos[1] + 'px';
			}
		...
		addDefaultButton: function () {
			...
			for (let buttonOption of this.options.view.defaultButton) {
				...
				//添加SVG元素
				let svgDom = parseSVG(buttonOption.svgStr, this.options.view.defaultButtonColor);
