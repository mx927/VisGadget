...
(function (undefined) {
	...
	VisGadget.prototype = {
		...
		_initial: function (opt) {
			...
			// 合并toolbox参数
			if (!!opt.view) {
				opt.view = extend(opt.view, options.view, false);
			}
		...
		createView: function () {
			...
			for (let btn of DEF_BTN) {
				this.addDefaultButton(btn);
			}
		...
		addDefaultButton:function(btnOption){
			...
			//添加SVG元素
			let svgDom = parseSVG(btnOption.svgStr,DEF_BTN_COLOR);
