...
class Monitor {
    ...
    getInclusion(selector) {
        ...
        for (let circle of allCircle) {
            ...
            if (selector.contain(p1[0], p1[1]) && selector.contain(p2[0], p2[1]) &&
                selector.contain(p3[0], p3[1]) && selector.contain(p4[0], p4[1])) {
                shape.attr({
                    style: {
                        fill: selectCircle
                    }
    ...
    _rectMode_ready() {
        ...
        let _this = this;
        _this.container.onmousedown = function (ev) {
            _this.currentSelectArea = new Monitor.zrender.Rect({
                type: 'rect',
                monitor: _this,
                zlevel:1,
                afterUpdate: _this._rectMode_afterUpdate,
                shape: {
                    x: ev.offsetX,
                    y: ev.offsetY,
                    width: 1,
                ...
                style: {
                    fill: _this.selectAreaColor,
                    opacity: _this.selectAreaOpacity,
                    stroke: 'black'
                }
            });
            _this.zr.add(_this.currentSelectArea);
            _this.container.onmousedown = null;
            _this._rectMode_processing();
        }
    }
     ...
     * 矩形模式：绘制状态
     */
    _rectMode_processing() {
        let _this = this;

        _this.container.onmousemove = function (ev) {
            let x = _this.currentSelectArea.shape.x;
            let y = _this.currentSelectArea.shape.y;
        ...
        //延迟 解决总是先触发mousemove、mouseout的奇异事件
        setTimeout(() => {
            _this.container.onmouseup = _this.container.onmouseout = function (ev) {
                let x = _this.currentSelectArea.shape.x;
                let y = _this.currentSelectArea.shape.y;
                let width = _this.currentSelectArea.shape.width;
                let height = _this.currentSelectArea.shape.height;
                ...
                _this.zr.remove(_this.currentSelectArea);
                if (width > 8 && height > 8) {
                    _this.selectArea.push(_this.currentSelectArea);
                    _this.zr.add(_this.currentSelectArea);
                }
                _this.getInclusion(_this.currentSelectArea);
                _this.container.onmousemove = null;
                _this.container.onmouseup = null;
                _this.container.onmouseout = null;
                _this.currentSelectArea = null;

                _this._rectMode_complete();
            }
    ...
    _ellipseMode_ready() {
        ...
        this.container.onmousedown = function (ev) {
            _this.currentSelectArea = new Monitor.zrender.Ellipse({
                monitor: _this,
                afterUpdate: _this._ellipseMode_afterUpdate,
                zlevel:1,
                shape: {
                    ox: ev.offsetX,
                    oy: ev.offsetY,
     ...
     * 椭圆模式: 绘制状态
     */
    _ellipseMode_processing() {
        let _this = this;

        _this.container.onmousemove = function (ev) {
            let sp = _this.currentSelectArea.shape;
        ...
        setTimeout(() => {
            _this.container.onmouseup = _this.container.onmouseout = function (ev) {
                let rx = _this.currentSelectArea.shape.rx;
                let ry = _this.currentSelectArea.shape.ry;
